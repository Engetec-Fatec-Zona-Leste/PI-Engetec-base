...
router.post('/register/user', async (req, res) => {
  const { email, nome, senha, cpf, periodo, apresentador, curso, instituicao } =
    req.body;
  const cargos = ['Autor'];
  ...
  if (invalidcargo.length > 0) {
    return res
      .status(400)
      .json({ message: `Invalid cargo: ${invalidcargo.join(', ')}` });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error registering user', error: error.message }); // Alterado para 500 para refletir erro interno
  }
});
