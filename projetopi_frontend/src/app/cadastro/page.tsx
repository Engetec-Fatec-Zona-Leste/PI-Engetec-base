...
export default function CadastroUser() {
  ...
  useEffect(() => {
    const fetchInstituicoes = async () => {
      try {
        const response = await baseURL.get<ResponseData>('/instituicao/aprovada');

        // Verificando a estrutura da resposta
        console.log(response.data); // Isso deve mostrar a estrutura completa da resposta

        // Verificando se a chave 'instituicoes' existe e é um array
        if (response.data && Array.isArray(response.data.instituicoes)) {
          const instituicoesOptions = response.data.instituicoes.map((instituicao) => ({
            label: instituicao.nome,  // Nome da instituição
            value: instituicao.id,    // ID da instituição
          }));
